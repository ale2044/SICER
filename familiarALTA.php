...
$cuil = $_GET["titular"];
		...
		$sqlBusca="select cuil from titudesdefilial where cuil='$cuil'";
		$resultBusca=mysql_query($sqlBusca, $link);

		if(mysql_num_rows($resultBusca)==1){
			$pcias=mysql_query("SELECT * from titudesdefilial WHERE cuil='$cuil' ");
			$row=mysql_fetch_assoc($pcias);
		}
		else {
			$alerta=411; //El empleado NO existe;
...
<form action="graba_fami.php" name="formulario" id="formulario" method="POST">
